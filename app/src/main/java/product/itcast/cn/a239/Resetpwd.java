...
//        layout.setOrientation(RelativeLayout.VERTICAL).
        ...
        chahkanButton= (Button) findViewById(R.id.resetpwd_chakanxinxi);
        ...
        mAccount.setText(a);
       // mPwd_old.setText(mUserDataManager.find(a).getUserPwd());
        //mPwd_old.setText(mUserDataManager.find().getUserPwd());

        mSureButton.setOnClickListener(m_resetpwd_Listener);      //注册界面两个按钮的监听事件
        mCancelButton.setOnClickListener(m_resetpwd_Listener);
        //mCancelButton.setOnClickListener(m_resetpwd_Listener);
        chahkanButton.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    public void passDate(){
        ...
        intent.putExtra("password", mPwd_old.getText().toString().trim());
        intent.putExtra("password1", mPwd_new.getText().toString().trim());
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.resetpwd_btn_cancel:                     //取消按钮的监听事件,由注册界面返回登录界面
                    Intent intent_Resetpwd_to_Login = new Intent(Resetpwd.this,main_Activity.class) ;    //切换Resetpwd Activity至Login Activity
